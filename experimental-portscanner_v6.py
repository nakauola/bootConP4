...
def quickscan():
    pool = multiprocessing.Pool(processes=len(port))
    
    # Perform port scanning using multiprocessing
    results = []
    for port in port:
        result = pool.apply_async(port_scan, args=(host, port))
        results.append(result)
    
    # Retrieve results from the async tasks
